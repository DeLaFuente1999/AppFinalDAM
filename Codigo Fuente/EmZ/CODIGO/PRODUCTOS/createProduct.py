...
while True:             
    ...
    if callable(event):
        ...
        datos = [values['productName'], values['productDescription'], values['providerName'].split(',')[1].strip().split(':')[1], values['productReference'], str(values['productPrice']) ,str(values['test'].split('/')[-1:])[2:-2]]
