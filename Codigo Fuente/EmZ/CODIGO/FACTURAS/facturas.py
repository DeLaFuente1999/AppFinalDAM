...
def generarFactura():
    ...
    with open('%s\\factura_%s.txt' % (values['ruta'], dt_string) , 'w') as f:
        ...
        for i in productos:
            valorFinal = valorFinal + float(i[1])
...
while True:             # Event Loop
    ...
    if callable(event):
            ...
            if len(productos) == 0:
                ...
                for i in productos:
                    if i[0] == items1[values['items'][0]][0]:
                        i[2] = i[2] + 1
                        i[1] = round(float(items1[values['items'][0]][1]) * i[2], 2)
                        break
                else:
                    productos.append([items1[values['items'][0]][0],items1[values['items'][0]][1],1])
            window['items2'].update(productos)
        ...
        if len(values['items2']) == 0:
            ...
            productos[values['items2'][0]][1] = round((float(productos[values['items2'][0]][1]) - float(items1[values['items2'][0]][1])), 2)
